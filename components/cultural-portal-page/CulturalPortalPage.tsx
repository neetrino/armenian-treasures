...
async function CulturalPortalDeferredSections({
  ...
  const visibility = pageContent.sectionVisibility;
  const loadProjects = isSectionEnabled(visibility, 'projects');
  ...
  const portalProjects = loadProjects ? mapProjectsToCulturalPortalProjects(projects) : [];
  ...
  return (
    <LandingSectionStack>
      ...
      {loadDonors && donorGroups.length > 0 ? (
      ...
      {loadAbout && about && aboutCards.length > 0 ? (
        <CulturalPortalAbout
          section={{
