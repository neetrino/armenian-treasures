...
export function CulturalCategoryCard({ category }: CulturalCategoryCardProps) {
  return (
    <Link href={category.href} className="cultural-portal-card group block h-full outline-none">
      <CulturalCategoryIcon type={category.icon} />

      <h3 className="cultural-portal-card__title mt-[22px] mb-[9px] max-w-[9.375rem] font-cinzel text-[12px] font-extrabold uppercase leading-[1.15] tracking-[0.04em] sm:text-[13px] sm:leading-[1.2]">
        {category.title}
      </h3>

      <p className="cultural-portal-card__description max-w-[9.375rem] font-display text-[12px] leading-[1.35] sm:text-[13px]">
        {category.description}
      </p>

      <span className="cultural-portal-card__arrow" aria-hidden>
        →
      </span>
    </Link>
  );
