...
export function AdminTable<T>({
  ...
  return (
    ...
    >
      <table className="w-full table-fixed divide-y divide-stone-200/60 text-sm">
        <thead className="bg-gradient-to-r from-parchment-100/90 to-parchment-50/80 text-left text-[11px] font-semibold uppercase tracking-[0.08em] text-ink-muted">
          <tr>
            {columns.map((col) => (
              <th
                key={col.key}
            ...
            >
              {columns.map((col) => (
                <td
                  key={col.key}
                  className={cn(
                    'min-w-0 px-4 py-3 align-middle text-ink',
