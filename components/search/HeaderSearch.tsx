...
export function HeaderSearch({ locale = 'EN' }: HeaderSearchProps) {
  ...
  return (
    <div ref={rootRef} className="relative shrink-0">
      <button
        ...
        aria-label={uiMessage(locale, 'searchCatalog')}
      ...
      {open ? (
        <div
          ...
          role="dialog"
          aria-label={uiMessage(locale, 'searchPlaceholder')}
        ...
        >
          <form action={CATALOG_SEARCH_PATH} method="get" className="flex flex-col gap-3">
            ...
            >
              {uiMessage(locale, 'searchCatalogCta')}
            </button>
          </form>
        </div>
      ) : null}
